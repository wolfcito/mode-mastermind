  ...
  describe("ModeMasterMind", function () {
    ...
    if (contractAddress) {
      it("Should connect to external contract", async function () {
        myContract = await ethers.getContractAt("ModeMasterMind", contractAddress);
        console.log("     🛰 Connected to external contract", myContract.address);
      });
    } else {
      it("Should deploy ModeMasterMind", async function () {
        const ModeMasterMind = await ethers.getContractFactory("ModeMasterMind");
        myContract = await ModeMasterMind.deploy();
      });
    }
